import type { EslintConfig } from './types.ts';
...
const mergeValue = (left: unknown, right: unknown): unknown => {
  ...
  if (isPlainObject(left) && isPlainObject(right)) {
    ...
    return [...keys].reduce<EslintConfig>((acc, key) => {
...
export const mergeEslintOptions = (left: EslintConfig, right: EslintConfig): EslintConfig => {
